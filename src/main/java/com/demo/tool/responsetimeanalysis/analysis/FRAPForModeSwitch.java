...
/**  For Mode Switch
 *  1. LO tasks interference
 *  2. all_csl_queue
 *  3. local blocking ck -> from Hi - ck_hi or only from LO - ck_lo
 *  **/
public class FRAPForModeSwitch {

    private ArrayList<ArrayList<Pair<Long, Long>>> RLOP = null;
    ...
    private long[][] busyWindow(ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, ArrayList<ArrayList<SporadicTask>> lowTasks, long[][] response_time, double oneMig, long np,
    ...
    private long oneCalculation(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, ArrayList<ArrayList<SporadicTask>> lowTasks, long[][] response_time, long Ri,
        ...
        // 计算B,S
        task.local = getS$B_Min_C_Max_F(task, tasks, resources, lowTasks, response_time, Ri, btbHit, useRi);
    ...
    private long getS$B_Min_C_Max_F(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, ArrayList<ArrayList<SporadicTask>> lowTasks,
                                    long[][] Ris, long time,
        ...
        for (int i = 0; i < lowTasks.get(task.partition).size(); i++) {
            ...
            if (high_task_priority > task.priority) {
                // 在这个高优先级任务的'优先级'下，抢占次数
                long preemptions = (long) Math.ceil((double)(time) / (double) lowTasks.get(task.partition).get(i).period);
                pri_preempt.put(high_task_priority,preemptions);
            }
        }
        ...
        for(int i = 0 ; i < this.RLOP.size(); i++){
            ArrayList<Pair<Long, Long>> rlorp = new ArrayList<>(this.RLOP.get(i));  // 该资源在每个处理器上的剩余次数
...
//                        resources.get(i).id, rlorp.size()));
                ...
                for(int q = 0; q < rlorp.size(); q++){
                    Pair<Long, Long> remote_request = rlorp.get(q);
                    if (remote_request.getFirst()>0){
                        blocking += resources.get(i).csl_high;
                        rlorp.get(q).setFirst(remote_request.getFirst() - 1);
                        if (rlorp.get(q).getFirst() < 1 && rlorp.get(q).getSecond() < 1){
                            rlorp.remove(q);
                            q--;
                        }
                    ...
                    if (remote_request.getSecond() > 0){
                        blocking += resources.get(i).csl_low;
                        rlorp.get(q).setSecond(remote_request.getSecond() - 1);
                        if (rlorp.get(q).getSecond() < 1){
                            rlorp.remove(q);
                            q--;
                        }
                    }
...
//                }
        ...
        ArrayList<SporadicTask> localTasks = new ArrayList<>();
        localTasks.addAll(tasks.get(partition));
        localTasks.addAll(lowTasks.get(partition));
        ...
        for (int i = 0; i < resources.size(); i++) {
            ...
            if (resource.partitions.size() == 1 && resource.partitions.get(0) == partition
                ...
                for (int j = 0; j < resource.requested_tasks.size(); j++) {  /**需要检查该数组  初始包括了lo和hi， 但是高关键模式需要去掉低关键任务  **/
                    SporadicTask LP_task = resource.requested_tasks.get(j);
                    if (LP_task.partition == partition && LP_task.priority < task.priority) {
                        // 本地资源必然只有一个小c
                        //rIndexList_B_Onlyc.add(resource.id - 1);
                        isAdd = true;
                        if (LP_task.critical == 1)  resource.onlyLow = false; // 要在外循环判断 这里会break
                        //break;
                    }
                }
            }
...
//        return spin;
    ...
    private long FIFOPResourceAccessTime(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, ArrayList<ArrayList<SporadicTask>> lowTasks,
        ...
        ArrayList<ArrayList<Pair<Long, Long>>> requestsLeftOnRemoteP = new ArrayList<>();
    ...
    private long getSpinDelayForOneResoruce(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, Resource resource, ArrayList<ArrayList<SporadicTask>> lowTasks,
                                            ArrayList<Pair<Long, Long>> requestsLeftOnRemoteP, long[][] Ris, long time, boolean btbHit, boolean useRi) {

        // 需要注意的前提是，此函数计算的是针对'单个'资源计算相应的访问&阻塞时间

        long spin = 0;
        long ncs = 0;   //访问该资源的所有'本地'高优先级任务在task的响应时间内的访问次数之和
        Pair<Long, Long> local_ncs = new Pair<Long, Long>();
...
//                        * hpTask.number_of_access_in_one_release.get(hpTask.resource_required_index.indexOf(resource.id - 1));
        ...
        // (hi, lo) request number
        local_ncs.setFirst(ncs);
        ...
        for (int i = 0; i < lowTasks.get(task.partition).size(); i++) {
            ...
            if (hpTask.priority > task.priority && hpTask.resource_required_index.contains(resource.id - 1)) {
                // task的Ri_LO内，hp任务访问的次数，未加btb
                ncs += (long) Math.ceil((double) (task.Ri_LO) / (double) hpTask.period) * hpTask.number_of_access_in_one_release.get(hpTask.resource_required_index.indexOf(resource.id - 1));
            }
        }
        local_ncs.setSecond(ncs);
        ...
        for (int i = 0; i < tasks.size(); i++) {
            ...
            if (task.partition != i) {
                ...
                Pair<Long, Long> remote_request = new Pair<Long, Long>();
                ...
                remote_request.setFirst(number_of_request_by_Remote_P);

                number_of_request_by_Remote_P = 0;
                // 遍历该处理器上的低关键任务
                for (int j = 0; j < lowTasks.get(i).size(); j++) {
                    ...
                    if (lowTasks.get(i).get(j).resource_required_index.contains(resource.id - 1)) {
                        ...
                        // 在task的响应时间内远程处理器访问该资源的总次数
                        int number_of_release = (int) Math
                                .ceil((double) (task.Ri_LO + (btbHit ? (useRi ? remote_task.Ri_LO : remote_task.deadline) : 0)) / (double) remote_task.period);
                        number_of_request_by_Remote_P += (long) number_of_release * remote_task.number_of_access_in_one_release.get(indexR);
                    }
                }
                remote_request.setSecond(number_of_request_by_Remote_P);

                // 取number_of_request_by_Remote_P和ncs的最小值，作为Direct/InDirect Spin Delay的bound。
                long possible_spin = Long.min(local_ncs.getFirst()+local_ncs.getSecond(), remote_request.getFirst());

                spin += possible_spin * resource.csl_high;
                if (local_ncs.getFirst()+local_ncs.getSecond() > remote_request.getFirst()){
                    long delay = Long.min(local_ncs.getFirst()+local_ncs.getSecond()-remote_request.getFirst(), remote_request.getSecond());
                    spin += delay * resource.csl_low;
                    remote_request.setFirst(0L);
                    remote_request.setSecond(remote_request.getSecond()-delay);
                }else{
                    remote_request.setFirst(remote_request.getFirst()-possible_spin);
                }
                // 如果该处理器上还有处理次数，更新requestsLeftOnRemoteP。
                if (remote_request.getFirst()>0 || remote_request.getSecond() > 0)
                    requestsLeftOnRemoteP.add(remote_request);
...
//                        requestsLeftOnRemoteP.add(number_of_request_by_Remote_P - ncs);
        ...
        return spin + local_ncs.getFirst()*resource.csl_high + local_ncs.getSecond()*resource.csl_low;
    }

    private long highPriorityInterference(SporadicTask t, ArrayList<ArrayList<SporadicTask>> allTasks, ArrayList<Resource> resources, ArrayList<ArrayList<SporadicTask>> lowTasks, long[][] Ris, long time,
                                          double oneMig, long np, boolean btbHit, boolean useRi) {
        long interference = 0;
        ...
        tasks = lowTasks.get(partition);
        for (int i = 0; i < tasks.size(); i++) {
            if (tasks.get(i).priority > t.priority) {
                SporadicTask hpTask = tasks.get(i);
                interference += (long) Math.ceil((double) (t.Ri_LO) / (double) hpTask.period) * (hpTask.WCET);
                // t.implementation_overheads += Math.ceil((double) (time) / (double) hpTask.period) * (AnalysisUtils.FULL_CONTEXT_SWTICH2);
            }
        }
